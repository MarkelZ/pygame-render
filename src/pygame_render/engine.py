...
class RenderEngine:
    ...
    def __init__(self, screen_width: int, screen_height: int,
        ...
        # Create draw shader program
        prog_tonemap = self._ctx.program(vertex_shader=vertex_src,
                                      fragment_shader=fragment_src_tonemap)
        ...
        fragment_src_text = resources.read_text(
            'pygame_render', 'fragment_text.glsl')

        # Create draw shader program
        prog_text = self._ctx.program(vertex_shader=vertex_src,
    ...
    def render_text(self, layer, text: str, letter_frame: int, color: tuple = (1.0, 1.0, 1.0, 1.0), scale: float = 1.0):
        """
        ...
        self.render_char_batch(layer, vertices, tex_coords, color)

        return layer

    # same as render_text but supports alignment. Is alignment needed?
    def render_text_aligned(self, layer, text: str, letter_frame: int, color: tuple = (1.0, 1.0, 1.0, 1.0), scale: float = 1.0, alignment: str = "left"):
        """
        Render the text on the specified layer with an optional color.
    ...
    def render_char_batch(self, layer, vertices, tex_coords, color):
        """
        Renders a batch of characters using a single draw call and applies a color.
        
        Parameters:
        - layer: The rendering layer to draw on.
        - vertices: The vertices for the character quads.
        - tex_coords: The texture coordinates for the character quads.
        - color: The color to apply to the text as an RGBA tuple.
        """
        ...
        self._shader_text.program['textColor'].value = color # Pass the color to the shader
