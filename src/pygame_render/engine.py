...
class RenderEngine:
    ...
    def make_fontatlas(self, font_path: str = None, font_size: int = 64) -> FontAtlas:
    ...
    def render_text(self, font_atlas: FontAtlas, layer: Layer, text: str, 
                    letter_frame: int, color: tuple = (1.0, 1.0, 1.0, 1.0), 
                    scale: float = 1.0, alignment: str = None):
        """
        ...
        if alignment == None:
            vertices = font_atlas.get_char_batch(layer.width, layer.height, text, letter_frame, scale)
        else:
            vertices = font_atlas.get_char_batch_aligned(layer.width, layer.height, text, letter_frame, scale, alignment)
